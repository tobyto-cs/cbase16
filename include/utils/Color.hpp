...
struct rgb_hex : public rgb_base
{
  ...
  rgb_hex(const string r, const string g, const string b) : r(r), g(g), b(b) {}
  const string getstr() { return string("#")+r+g+b; }
...
struct rgb_dec : public rgb_base
{
  ...
  const string getstr() { return fmt::format("r={:.2f}, g={:.2f}, b={:.2f}", r, g, b); }
};
struct rgb_int : public rgb_base
{
  ...
  const string getstr() { return fmt::format("r={}, g={}, b={}", r, g, b); }
};
struct hsl_dec : public rgb_base
{
  ...
  const string getstr() { return fmt::format("h={:.2f}, s={:.2f}, l={:.2f}", h, s, l); }
};
...
class Color {
  ...
  private:
    ...
    template<typename T>
    static string numtostring(T x) {
      return fmt::format("{:x}", x);
    }
  public:
    static ushort hextoint(const char* n) { return std::stoul(n, nullptr, 16); };
    static ushort hextoint(const string n) { return std::stoul(n, nullptr, 16); };
    static ushort dectoint(float n) { return n * 255; };
    static string inttohex(ushort n) { return numtostring(n); }
    ...
    Color(const string& rgb) {
      string buf = rgb;
      if (buf.find('#') != string::npos) buf = buf.substr(1);
    ...
    bool operator==(const Color other) {
      std::cout << string("r[")+std::to_string(r)+"=="+std::to_string(other.r) +
